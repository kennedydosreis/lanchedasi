!function() {
  try {
    ...
    n && (e._sentryDebugIds = e._sentryDebugIds || {}, e._sentryDebugIds[n] = "30726099-cd5d-451f-8d32-dcfbb29d9f6b", e._sentryDebugIdIdentifier = "sentry-dbid-30726099-cd5d-451f-8d32-dcfbb29d9f6b");
  } catch (e2) {
  }
}();
...
function get_data(event, event_state, options2, nodes, csp, global2) {
  let promise_id = 1;
  let count = 0;
    ...
    if (typeof thing?.then === "function") {
      ...
      thing.then(
        ...
        async ({ data, error: error2 }) => {
          ...
          push(
            `<script${nonce}>${global2}.resolve(${id}, ${str.includes("app.decode") ? `(app) => ${str}` : `() => ${str}`})<\/script>
`
          );
          if (count === 0) done();
        }
